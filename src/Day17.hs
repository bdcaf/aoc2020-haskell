...
prettyCube c = putStr $ concatMap sliceZ (rv toZ cl)
  where sliceZ z = '[':show z ++ "]\n" ++ concatMap (sliceY z) (rv toY cl)

        sliceY z y =
          map (\x -> pretty $ (z,y,x) `S.member` c) (rv toX cl) ++ "\n"
...
neighbors !cc !k =
  ...
  where allN = length . filter (`isNeighbor` k) $ S.toList cc
...
expand4 !c0 = S.fromList c1
  where c1 = concatMap el (S.toList c0)
        ...
        el (a,b,c,d) =
          [(a + i,b + j,c + k,d + l) | i <- l1,j <- l1,k <- l1,l <- l1]
...
evolve4 !cc = seq cc2 $ seq cand2 $ cc2 `S.union` cand2
  where cand = cand4 cc

        newCand !co = neighbor4 cc co == 3

        keepCand !co =
          let n = neighbor4 cc co
          in n == 3 || n == 2
...
part1 :: IO ()
part1 = do print (sol1 $ readInput input)

part2 :: IO ()
part2 = do print (sol2 $ readInput input)
